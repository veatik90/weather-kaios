...
const weatherDiagram = ({ coords, temps }) => {
  ...
  return (
    <>
      ...
      <Temps>
        { isFinite(temps.night) ? (
          <Temp bottom={coords.night + 3}>
            {temps.night}
            &deg;
          </Temp>
        ) : emptyTemp}
        { isFinite(temps.morning) ? (
          <Temp bottom={coords.morning + 3}>
            {temps.morning}
            &deg;
          </Temp>
        ) : emptyTemp}
        { isFinite(temps.afternoon) ? (
          <Temp bottom={coords.afternoon + 3}>
            {temps.afternoon}
&deg;
          </Temp>
        ) : emptyTemp}
        { isFinite(temps.evening) ? (
          <Temp bottom={coords.evening + 3}>
            {temps.evening}
&deg;
          </Temp>
        ) : emptyTemp}
